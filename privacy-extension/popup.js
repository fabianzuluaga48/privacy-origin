...
function computeBuckets(requests) {
  ...
  const numBuckets = Math.max(
    1,
    Math.ceil((now - alignedStart) / bucketSizeMs)
  );
...
function renderNetworkGraph(buckets, labels, bucketSizeMs) {
  ...
  legend.innerHTML = `
    ...
    <div><span class="legend-average">Avg: ${avgPerBucket} per ${
    bucketSizeMs / 3600_000
  }h</span></div>
  `;
...
function updateUI() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.storage.local.get(
      ...
      (result) => {
        const filterByTab = (arr) =>
          (arr || []).filter((item) => item.tabId === currentTabId);
        ...
        try {
          ...
          document.getElementById("network-graph-section").style.display =
            "none";
        }
      }
    );
...
function updateTips(data) {
  ...
  if (data.fingerprinting.length > 0) {
    ...
    tips.push({
      icon: "⚠️",
      level: "danger",
      text: `Fingerprinting detected (${methods.join(
        ", "
      )}). This site may be creating a unique identifier for your device without cookies.`,
    });
  }
  ...
  if (data.formData.length > 0) {
    const submissions = data.formData.filter(
      (f) => f.action === "submit"
    ).length;
