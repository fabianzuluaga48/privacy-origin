...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === "loading" && changeInfo.url) {
    chrome.storage.local.get(
      ...
      (result) => {
        const clean = (arr) =>
          (arr || []).filter((item) => item.tabId !== tabId);
...
chrome.webRequest.onBeforeRequest.addListener(
  (details) => {
    ...
    try {
      ...
      initiatorHost = details.initiator
  ...
  if (message.type === "GEOLOCATION_ATTEMPT") {
    chrome.storage.local.get(["geolocationAttempts"], (result) => {
      ...
      const recentAttempt = attempts.find(
        (a) => a.tabId === tabId && Date.now() - a.timeStamp < 5000
      );
